...
public class Enemy    
{
  ...
  public void draw(){
	  GameState.draw.setPenColor(Draw.RED);
	  GameState.draw.filledRectangle(xCoord,yCoord,height,width);
   }
  public void vel(){
  ...
  (topEdgeRec1) < (bottomEdgeRec2)|| (topEdgeRec2) < (bottomEdgeRec1)){
   ...
   return isInterSecting;
	...
	public boolean clickedAtEnemy(double mouseX,double mouseY) { //function that checks if you clicked on a window

		if(mouseX>this.xCoord -this.width && mouseX < this.xCoord +this.width 
		&& mouseY>this.yCoord-this.height && mouseY < this.yCoord +this.height ) {
			return true;
		}
	return false;
	}
